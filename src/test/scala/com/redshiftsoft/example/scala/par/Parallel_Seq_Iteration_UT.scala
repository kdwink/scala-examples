...
class Parallel_Seq_Iteration_UT:
  ...
  @Test def par_array_set_thread_count(): Unit =
    ...
    // when
    val distinct: ParArray[String] = array.map(i => Thread.currentThread().getName).distinct

    // then
    assertEquals("ForkJoinPool-2-worker-1,ForkJoinPool-2-worker-2", distinct.toList.sorted.mkString(","));
